...
const SearchBar = () => {
  const { searchQuery, setSearchQuery, filteredProducts, categories, products } = useProducts();
  const [selectedCat, setSelectedCat] = useState('');

  // Kategoriyə görə filter + search
  const displayProducts = filteredProducts.filter(p =>
    selectedCat ? p.cat === selectedCat : true
  );

  // Unikal kateqoriya adları
  ...
  return (
    <div className="hidden min-[993px]:flex grow max-w-162.5 items-center bg-[#F5F5F5] rounded-full px-5 py-2.5 mx-8 relative">
      <select
        className="bg-transparent text-sm text-gray-600 outline-none pr-3 border-r border-gray-300 mr-3 cursor-pointer"
        value={selectedCat}
